SimpleNavigation::Configuration.run do |navigation|  
  navigation.items do |primary|
    primary.item :profile, Proc.new{ image_tag(current_user.avatar.url(:icon)) if current_user }.call, user_root_path, :class => "rahmen_profilbild", :if => Proc.new{ user_signed_in? }
