namespace :db do
  desc "Create, download, import and anonymize staging database dump into local db"
  task :clone_staging => %w(db:create_dump db:import_dump db:anonymise_dump)
  
  desc "Create and download staging db dump remotely"
  task :create_dump do
    Bundler.with_clean_env{ sh "cap staging db:backup" }
  end
  ...
  task :import_dump do
    filename = "piupiu_staging.dump.sql.bz2"
